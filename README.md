...
- **Chapters 2–14** (Polynomials through Probability — the full CBSE syllabus list — Constructions was removed from the current 2025-26 syllabus) exist as ready `meta.json` placeholders. Drop lesson JSON files into any of them and they go live instantly.
...
Only **content** — not code. Chapters 2–14 have no lesson JSON yet (that's real subject-matter writing, ~6-11 lessons × 14 chapters). The engine that renders them is 100% done; it's purely a content-authoring task from here, following `CONTENT-GUIDE.md`. I'd recommend authoring 2-3 chapters at a time and testing — happy to help write the JSON content itself in a follow-up if useful.
